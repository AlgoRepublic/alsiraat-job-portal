...
import { ArrowLeft, MapPin, Clock, Calendar, Download, CheckCircle, Upload, FileText, Loader2, Users, ShieldCheck, XCircle } from 'lucide-react';
...
export const JobDetails: React.FC = () => {
    ...
    const [applicationStep, setApplicationStep] = useState<'form' | 'submitting' | 'success' | 'applied'>('form');
        ...
        const loadJob = async () => {
            if (id) {
                ...
                if (found) {
                    ...
                    const hasApplied = await db.hasUserApplied(currentUser.id, id);
                    if (hasApplied) {
    ...
    const handleApply = async (e: React.FormEvent) => {
        ...
        await db.applyForJob(newApplication);
        setApplicationStep('success');
    };

    const handleManagerAction = async (action: 'approve' | 'decline') => {
        if(!job) return;
        const newStatus = action === 'approve' ? JobStatus.PUBLISHED : JobStatus.DECLINED;
        await db.updateJobStatus(job.id, newStatus);
        setJob({ ...job, status: newStatus });
    };

    if (loading) {
        return <div className="p-8 text-center text-zinc-500 dark:text-zinc-400 animate-pulse">Loading job details...</div>;
    }

    if (!job) {
        return <div className="p-8 text-center text-zinc-500 dark:text-zinc-400">Job not found.</div>;
    }

    const isManagerOrAdmin = currentUserRole === UserRole.MANAGER || currentUserRole === UserRole.ADMIN;
    // const isCreator = job.createdBy === currentUser.name; // In real app use ID
    const showManagerActions = isManagerOrAdmin && job.status === JobStatus.PENDING;
        ...
        <div className="max-w-5xl mx-auto space-y-8 animate-fade-in pb-20">
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <span className={`px-2.5 py-1 text-xs font-bold rounded-md uppercase tracking-wide border ${
                                job.status === JobStatus.PUBLISHED ? 'bg-emerald-100 text-emerald-800 border-emerald-200' : 
                                job.status === JobStatus.PENDING ? 'bg-amber-100 text-amber-800 border-amber-200' : 
                                'bg-zinc-100 text-zinc-600 border-zinc-200'
                            }`}>
                                {job.status}
                            </span>
                        </div>
                        <h1 className="text-3xl md:text-4xl font-bold text-zinc-900 dark:text-white">{job.title}</h1>
                    </div>
                    <div className="flex flex-col items-end">
                        <span className="text-2xl font-bold text-zinc-900 dark:text-white">
                            {job.rewardType === RewardType.PAID ? `$${job.rewardValue}` : 
                             job.rewardType === RewardType.VIA_POINTS ? `${job.rewardValue} Pts` : 
                             'Volunteer'}
                        </span>
                    </div>
                </div>
            ...
            {showManagerActions && (
                <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-900/50 p-6 rounded-2xl flex items-center justify-between">
                    <div>
                        <h3 className="text-lg font-bold text-amber-900 dark:text-amber-100">Review Required</h3>
                        <p className="text-sm text-amber-800 dark:text-amber-200/80">This job posting is pending approval. Review details and take action.</p>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="glass-card rounded-2xl p-6 shadow-sm border border-zinc-100 dark:border-zinc-800 flex flex-wrap gap-6">
                        <div className="flex items-center">
                            ...
                            <div>
                                <p className="text-xs text-zinc-400 dark:text-zinc-500 uppercase font-bold">Location</p>
                        ...
                        <div className="flex items-center">
                            ...
                            <div>
                                <p className="text-xs text-zinc-400 dark:text-zinc-500 uppercase font-bold">Duration</p>
                                <p className="text-sm font-semibold text-zinc-900 dark:text-white">{job.hoursRequired} Hours</p>
                            </div>
                        </div>
                        <div className="flex items-center">
                            <div className="w-10 h-10 rounded-full bg-red-50 dark:bg-red-900/20 flex items-center justify-center mr-3 text-red-900 dark:text-red-400">
                            ...
                            <div>
                                <p className="text-xs text-zinc-400 dark:text-zinc-500 uppercase font-bold">Start Date</p>
                                <p className="text-sm font-semibold text-zinc-900 dark:text-white">{job.startDate || "ASAP"}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="glass-card rounded-2xl p-8 shadow-sm border border-zinc-100 dark:border-zinc-800">
                        ...
                        <div className="prose prose-zinc dark:prose-invert max-w-none text-zinc-600 dark:text-zinc-300 whitespace-pre-wrap leading-relaxed">
                         ...
                         {job.selectionCriteria && (
                             <div className="mt-8">
                                <h4 className="text-base font-bold text-zinc-900 dark:text-white mb-3">Key Selection Criteria</h4>
                ...
                <div className="lg:col-span-1">
                    <div className="sticky top-24 space-y-6">
                        {/* View Applicants Button (For Manager/Creator) */}
                                ...
                                {applicationStep === 'success' || applicationStep === 'applied' ? (
                                    <div className="p-8 text-center animate-fade-in">
                                        ...
                                        <h3 className="text-xl font-bold text-zinc-900 dark:text-white mb-2">
                                            {applicationStep === 'success' ? 'Application Sent!' : 'Already Applied'}
                                        </h3>
                                        <p className="text-zinc-500 dark:text-zinc-400 text-sm mb-6">
                                            {applicationStep === 'success' 
                                                ? "The Job Manager will review your details and contact you shortly."
                                        ...
                                        <div className="pt-2">
                                            <label className="flex items-start space-x-3 cursor-pointer group">
                                                ...
                                                <span className="text-xs text-zinc-500 dark:text-zinc-400 leading-snug group-hover:text-zinc-700 dark:group-hover:text-zinc-200">
                                                    I agree to the <a href="#" className="underline">Terms & Conditions</a> and confirm that my profile information is accurate.
                                                </span>
                                            </label>
                                        </div>
