...
export const AdminSettings: React.FC = () => {
  ...
  const permissions = [
    { name: "User Management", desc: "Create, edit, delete system users" },
    {
      ...
      desc: "Initialize and deploy new task designations",
    },
    {
      name: "Review Orchestration",
      desc: "Review and approve pending task deployments",
    },
            ...
            {roles.map((role, i) => (
              ...
              >
                ...
                <td className="px-8 py-6 text-right">
                  <button className="text-zinc-400 hover:text-red-900 dark:hover:text-red-400 transition-colors">
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid gap-6">
        {permissions.map((perm, i) => (
          <div
            key={i}
            className="glass-card p-6 rounded-2xl border-white/10 flex justify-between items-center group hover:-translate-y-1 transition-all"
          >
            <div>
            ...
            <button className="p-3 bg-white/50 dark:bg-zinc-800/50 rounded-xl text-zinc-400 hover:text-red-900 dark:hover:text-red-400 transition-colors group-hover:scale-110">
              <Edit2 className="w-4 h-4" />
            </button>
          </div>
        ))}
          ...
          <tbody className="divide-y divide-white/20 dark:divide-white/5">
            {categories.map((cat, i) => (
              ...
              >
                ...
                <td className="px-8 py-6 text-[10px] font-black uppercase tracking-widest text-zinc-500">
                  Internal
                </td>
                <td className="px-8 py-6 text-right">
                  <button className="text-zinc-400 hover:text-red-900 dark:hover:text-red-400 transition-colors">
